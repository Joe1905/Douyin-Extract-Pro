...
TEMP_ROOT = os.path.join(BASE_DIR, "temp_downloads")
...
async def resolve_final_url(short_url):
    ...
    try:
        ...
        # timeout=30.0: 增加超时时间
        # trust_env=False: 忽略系统环境变量中的代理设置，避免干扰
...
def extract_keyframes(media_path, frames_dir):
    ...
    if not check_ffmpeg_installed():
        print_error("系统未检测到 FFmpeg/FFprobe！请确保已安装并添加到系统环境变量 Path 中。")
        return []
    ...
    try:
        ...
        time_points = [{"time": 0, "suffix": "00"}, {"time": duration * 0.5, "suffix": "50"},
                       {"time": duration * 0.9, "suffix": "90"}]
        for tp in time_points:
            frame_name = f"frame_{tp['suffix']}.jpg"
            frame_path = os.path.join(frames_dir, frame_name)
            ffmpeg_cmd = ["ffmpeg", "-v", "error", "-ss", str(tp['time']), "-i", media_path, "-vframes", "1", "-q:v",
                          "2", "-y", frame_path]
        ...
        print_error("执行 FFmpeg 命令失败：找不到指定文件。请检查 FFmpeg 是否正确安装并配置了环境变量。")
        return []
    except Exception:
...
def split_audio(audio_path, chunks_dir):
    ...
    if not check_ffmpeg_installed():
        print_error("系统未检测到 FFmpeg，无法切分音频。")
        return []
    ...
    try:
        ...
        print_error("执行 FFmpeg 命令失败：找不到指定文件。请检查 FFmpeg 是否正确安装。")
        return []
    except Exception:
...
async def run_pipeline(video_url: str, config: dict):
    ...
    # 使用动态路径
    task_dir = os.path.abspath(os.path.join(TEMP_ROOT, ts))
    ...
    try:
        ...
        try:
            ...
            async with async_playwright() as p:
                ...
                context = await browser.new_context(user_agent=DESKTOP_USER_AGENT,
                ...
                async def handle_response(response):
                    try:
                        ...
                        if not any(c['url'] == response.url for c in candidate_media):
                            print_success(
                                f"捕获候选媒体: {response.url[:80]}... (Size: {clength / 1024 / 1024:.2f} MB)")
                ...
                while time.time() - start_wait < max_wait_time:
                    if len(candidate_media) > 0 and time.time() - start_wait > 3:
                        print_success("已捕获媒体资源，提前结束等待。")
                        break
                    yield json.dumps({"step": "loading_media",
                                      "message": f"[{get_timestamp()}] 正在嗅探媒体流 ({int(time.time() - start_wait)}/{max_wait_time}s)..."})
                    await asyncio.sleep(1)

                if not candidate_media:
                    raise ValueError("警告：超时未捕获到任何符合条件的媒体请求！可能是反爬虫生效或网络拥堵。")
                ...
                dl_kwargs = {
                    ...
                    "verify": False,
                    "timeout": 60.0,  # 增大下载超时
                    "trust_env": False  # 默认不信赖环境代理
                }
                # 只有当用户显式配置了代理时，才使用代理
                if config.get("proxyUrl"):
                    dl_kwargs["proxy"] = config.get("proxyUrl")
                    dl_kwargs["trust_env"] = True  # 如果配置了代理，则允许环境影响(其实这里proxy参数优先级更高)

                async with httpx.AsyncClient(**dl_kwargs) as client:
                    resp = await client.get(best['url'], follow_redirects=True)
        ...
        if not frames:
            raise ValueError("关键帧提取失败 (FFmpeg 未安装或运行错误)，无法继续。")
        ...
        if api_key:
            ...
            if chunks:
                ...
                if config.get("proxyUrl"):
                    client_kwargs["proxy"] = config.get("proxyUrl")
                ...
                raise ValueError("音频切片失败 (FFmpeg 问题)，无法进行 ASR。")
        else:
        ...
        result_data = {
            ...
            "frames": [f"{ts}/frames/{f}" for f in frames]
        }
